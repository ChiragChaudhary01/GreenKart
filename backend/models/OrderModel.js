...
const OrderModel = {
  ...
  getAllForDeliveryPerson: (delivery_person_id) => {
    ...
    return new Promise((resolve, reject) => {
      const sql =
        "select d.delivery_id, d.delivery_status, d.assigned_at, d.delivered_at, o.order_status, o.address_id, oi.quantity, oi.price, p.name, p.img_url, uf.user_name as farmer_name, uf.phone as farmer_phone, (CONCAT(af.address_line, ', ', af.city, ', ',af.state, ', ', af.pincode)) as farmer_address, uc.user_name as consumer_name, uc.phone as consumer_phone, (CONCAT(ac.address_line, ', ', ac.city, ', ',ac.state, ', ', ac.pincode)) as consumer_address from delivery d join orders o on d.order_id = o.order_id  left join order_items oi on o.order_id = oi.order_id  left join products p on oi.product_id = p.product_id left join users uf on uf.user_id = p.farmer_id left join address af on af.user_id = uf.user_id left join users uc on uc.user_id = o.consumer_id left join address ac on ac.address_id = o.address_id where d.delivery_person_id = ?";
